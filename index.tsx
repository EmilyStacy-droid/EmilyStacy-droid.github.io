...
const legacyPosts: BlogPost[] = [
  ...
  },
  {
    ...
    title: 'Archive: Hello World',
    date: '2019-01-15',
    excerpt: 'My first post from the old Jekyll site.',
    content: `# Welcome back.\n\nThis is content brought over from my original site.\n\n> "The best way to learn is to build."\n\nI'm keeping this here to track my growth from a student at the University of North Texas to a full-stack engineer.`
  }
];
...
const projectsData = [
  {
    ...
    description: "A comprehensive analysis of data structures and their performance in Spring-based environments, focusing on efficient sorting and graph traversals."
  },
  {
    title: "Interactive Learning Modules",
    tags: ["JavaScript", "UX", "Instructional Design"],
    impact: "Refined instructional logic.",
    image: "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&q=80&w=800",
    description: "Designing educational interfaces that apply Cognitive Load Theory to simplify complex coding concepts for technical learners."
  }
];
...
class GeminiService {
  ...
  async sendMessage(message: string): Promise<string> {
    ...
    } catch (error) { return "I'm having a technical hiccup. Please try again later!"; }
  }
}
    ...
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${isScrolled || currentPage !== 'home' ? 'glass py-3 shadow-sm' : 'bg-transparent py-6'}`}>
        ...
        <div className="hidden md:flex space-x-10 items-center">
          <ul className="flex space-x-10">
            {['projects', 'blog', 'skills'].map((id) => (
              <li key={id}>
                <button onClick={() => handleNav(id as Page)} className={`text-[12px] font-bold uppercase tracking-[0.2em] transition-all hover:text-[#2d4a3e] ${currentPage === id ? 'text-[#2d4a3e] border-b-2 border-[#84a59d] pb-1' : 'text-[#5c554a]'}`}>
                  {id === 'blog' ? 'The Log' : id}
                </button>
              </li>
            ))}
      ...
      {isMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-[#fffcf5] border-t border-[#e8f3ee] p-10 shadow-2xl flex flex-col space-y-8 animate-in slide-in-from-top duration-300">
          ...
          <a href="mailto:emilystacy2016@hotmail.com" className="text-left text-lg font-bold uppercase tracking-widest text-[#84a59d]">Contact</a>
        </div>
      )}
    </nav>
  );
  ...
  <div className="animate-in fade-in duration-1000">
    ...
    <section className="py-24 bg-white border-y border-[#e8f3ee]">
      <div className="max-w-5xl mx-auto px-6 flex flex-col md:flex-row gap-20 items-center">
        <div className="w-full md:w-1/3"><div className="aspect-[4/5] rounded-[3rem] overflow-hidden shadow-2xl border-4 border-[#fffcf5] transition-transform hover:rotate-2"><img src="https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&w=800" className="w-full h-full object-cover" /></div></div>
...
const ProjectsPage: React.FC = () => (
  <section className="pt-40 pb-32 min-h-screen bg-[#fffcf5] animate-in slide-in-from-bottom-6 duration-700">
    <div className="max-w-7xl mx-auto px-6">
      <div className="mb-24 text-center"><h2 className="text-[12px] font-bold uppercase tracking-[0.6em] text-[#84a59d] mb-4">The Portfolio</h2><p className="text-6xl text-[#2d4a3e] font-display font-bold">Project <span className="text-[#84a59d] font-light italic">Gallery.</span></p></div>
        ...
        {projectsData.map((p, i) => (
          <div key={i} className="group bg-white rounded-[3rem] overflow-hidden border border-[#e8f3ee] hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
            <div className="h-96 overflow-hidden relative">
              ...
              <div className="absolute top-10 left-10 flex flex-wrap gap-3">{p.tags.map(t => <span key={t} className="px-5 py-2 bg-[#2d4a3e]/90 backdrop-blur-md text-[10px] font-bold uppercase tracking-widest text-white rounded-full">{t}</span>)}</div>
            </div>
            <div className="p-14 space-y-8">
              ...
              <div className="pt-10 border-t border-[#e8f3ee] flex justify-between items-center">
                <div><span className="text-[10px] font-bold text-[#84a59d] uppercase tracking-widest block mb-1">Impact</span><p className="text-[16px] text-[#2d4a3e] font-bold">{p.impact}</p></div>
    ...
    <div className="fixed bottom-10 right-10 z-[60]">
      {isOpen ? (
        <div className="w-80 md:w-[420px] h-[550px] bg-white shadow-2xl rounded-[2.5rem] flex flex-col overflow-hidden border border-[#e8f3ee] animate-in zoom-in-95 duration-300">
          ...
          <div className="p-5 bg-white border-t border-[#e8f3ee] flex space-x-3"><input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Ask..." className="flex-1 bg-[#fffcf5] border border-[#e8f3ee] rounded-full px-6 text-sm focus:outline-none"/><button onClick={handleSend} className="p-4 bg-[#2d4a3e] text-white rounded-full">➜</button></div>
        </div>
  ...
  return (
    <div className="min-h-screen selection:bg-[#fde9e4] selection:text-[#2d4a3e]">
      ...
      <footer className="py-32 bg-[#fffcf5] border-t border-[#e8f3ee] text-center">
        <div className="max-w-4xl mx-auto px-6"><h2 className="text-4xl font-display font-bold mb-10 text-[#2d4a3e]">Let's Architect the Future.</h2><a href="mailto:emilystacy2016@hotmail.com" className="px-12 py-5 bg-[#2d4a3e] text-white rounded-full font-bold text-sm tracking-widest uppercase hover:bg-[#3d6353] shadow-2xl inline-block transition-all">Send an Email</a><div className="mt-32 pt-12 border-t border-[#e8f3ee] flex flex-col md:flex-row justify-between items-center text-[#84a59d] text-[11px] font-bold uppercase tracking-[0.4em]"><p>© {new Date().getFullYear()} Emily Stacy</p><div className="flex space-x-12 mt-10 md:mt-0"><a href="https://github.com/EmilyStacy-droid" target="_blank" className="hover:text-[#2d4a3e]">GitHub</a><a href="#" className="hover:text-[#2d4a3e]">LinkedIn</a></div></div></div>
      </footer>
      <AIChat />
    </div>
  );
